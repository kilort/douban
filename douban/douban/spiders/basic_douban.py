    ...
    def parse(self, response):
        if 'movie' in response.url:
            ...
            for movie_api in movie_api_list:
                yield Request(url=movie_api, callback=self.parse_detial,meta={'cookiejar':1},dont_filter=True)
            ...
            if all_urls:
                ...
                for this_url in all_urls:
                    ...
                    if key_url:
                        key_url = key_url[0]
                        yield Request(url=key_url, callback=self.parse_detial,meta={'cookiejar':1},dont_filter=True)
    ...
    def parse_detial(self,response):
        if 'movie' in response.url:
            ...
            for detial_url in movie_json["data"]:
                ...
                if detial_url:
                    yield Request(url =detial_url,callback=self.get_data,meta={'cookiejar':response.meta["cookiejar"],'tag1':tag1,'tag2':'{0}'.format(movie_tag)},dont_filter=True)
            ...
            if start:
                ...
                if next_movie_api:
                    yield Request(url=next_movie_api,callback=self.parse_detial,meta={'cookiejar':response.meta["cookiejar"]},dont_filter=True)
            ...
            if detial_url_list:
                for detial_url in detial_url_list:
                    ...
                    yield Request(url=detial_url,callback=self.get_data,meta={'cookiejar':response.meta["cookiejar"],'tag1':tag1,'tag2':'{0}'.format(anthor_tag)},dont_filter=True)
            next_url = response.xpath('//div[@class="paginator"]/span[@class="next"]/a/@href').extract()
            if next_url:
                next_url = parse.urljoin(response.url,next_url[0])
                yield Request(url=next_url,callback=self.parse_detial,meta={'cookiejar':response.meta["cookiejar"]},dont_filter=True)
